...
class TestLogging(unittest.TestCase):
    ...
    def test_log_model_loading(self, logger, caplog):
        start_time = 0
        end_time = 1000
    ...
    def test_log_processing(self, logger, caplog):
        file_count = [0, 1, 2, 3, 4]
        logger.log_processing(file_count)
        assert "Processing #5.." in caplog.text


    def test_log_file_start(self, logger, caplog, temp_log_dir):
    ...
    def test_log_file_end(self, logger, caplog, temp_log_dir):
    ...
    def test_log_finished(self, logger, caplog):
        filename = "test.txt"
